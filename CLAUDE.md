...
A Chrome Extension (Manifest V3) called "Gemini Positivity Filter". It scans text on any web page and, preferring Chrome's on-device `LanguageModel` (Gemini Nano) when available and falling back to the Gemini API otherwise, detects negative/toxic/low-value content and removes matching elements from the page.
...
popup.html
popup.js                    entry point: initializes the two popup sections on DOMContentLoaded
popup/
  apiKeySettings.js          initApiKeySettings() — Gemini API key field
...
- **`popup.js` + `popup/*`** — Settings UI with two independent sections, each initialized by `popup.js` on `DOMContentLoaded`:
  - `apiKeySettings.js`: reads/writes `chrome.storage.local.geminiApiKey`. This is the only place the key is set.
...
- **`content.js`** — **Currently double-registered**: `manifest.json` now statically declares it under `content_scripts` on `<all_urls>` (runs on every site unconditionally), *and* `background.js` still dynamically (un)registers it per-site via `lib/contentScriptSync.js`. `contentScriptSync.js`'s own header comment still says content.js is "no longer statically declared in manifest.json" — that's no longer true and looks like an unintentional regression from a recent `manifest.json` edit, not a deliberate design change. Until reconciled, the allowed-sites list in the popup does not actually gate where the filter runs. On page `load`, and again (debounced, via `scroll` + a `MutationObserver` on `document.body`, to catch lazy-loaded/infinite-scroll content) afterwards, `scanPage()` grabs unprocessed `p`/`h1`/`h2`/`h3`/`span`/`a` elements (tracked in a `WeakSet` so nothing is re-scanned) up to 50 per batch. For each one, it first tries Chrome's on-device `window.LanguageModel` (Gemini Nano) if `availability() === 'available'`, prompting it directly in-page; only if that's unavailable or errors does it fall back to sending a `{action: "checkText", text}` message to the background script. Either path funnels into `handleNegative()`, which finds the nearest sensible container (`[role="article"]`, `article`, `.post`, `.tweet`, `.card`, `.feed-item`, `li`, or a fallback `div`/`section`/parent) and **removes it from the DOM outright** — the blur-and-click-to-reveal styling further down in `handleNegative()` is dead code, unreachable because of an early `return` in the container-found branch.
- **`background.js` + `lib/*`** — MV3 module service worker with two responsibilities:
  1. **`lib/contentScriptSync.js`**: `syncContentScripts()` reads `allowedUrls` from storage and calls `chrome.scripting.registerContentScripts`/`unregisterContentScripts` to (re)inject `content.js` only into those site patterns. `background.js` runs it on `onInstalled`, `onStartup`, and whenever `chrome.storage.onChanged` fires for `allowedUrls` — so any popup edit takes effect without reloading the extension. Requires the `scripting` permission in `manifest.json`. See the `content.js` note above: this is currently redundant with (and overridden by) a static `content_scripts` entry in `manifest.json`.
...
Manifest permissions: `storage`, `scripting` (for dynamic content-script registration), `activeTab` (for popup's "Add This Website" button, see `allowedSites.js` above), and `host_permissions: ["<all_urls>"]` (needed so the dynamic registration can target whatever site the user adds, and for outbound fetch to `generativelanguage.googleapis.com`). `manifest.json` also currently has a static `content_scripts` entry injecting `content.js` on `<all_urls>` — see the "Currently double-registered" note above.
