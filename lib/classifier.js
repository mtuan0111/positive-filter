...
// Pulls the model's text output out of an Interactions API response. Tries a few
// plausible shapes since the exact field names were sourced from docs rather than a
// verified live response; returns null (instead of throwing) if none of them match.
function extractOutputText(data) {
  const modelStep = data.steps?.find(step => step.type === "model_output");
  const text =
    modelStep?.content?.[0]?.text ??
    modelStep?.content?.find?.(part => part.type === "text")?.text ??
    modelStep?.text ??
    data.output_text ??
...
export async function classifyText(textToCheck) {
  ...
  try {
    ...
    if (rawText === null) {
      // The Interactions API is new and this shape was sourced from docs, not a live
      // response -- log the raw body so the real shape can be confirmed and hard-coded.
      console.error("Gemini API Error: unrecognized response shape", JSON.stringify(data));
      return { error: "Unrecognized Gemini response shape." };
    }
